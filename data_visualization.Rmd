...
Dans l'atelier 6, nous avons fait nos PCA avec la fonction `rda`. Lorsqu'on compare le sommaire des deux analyses, on voit que les deux fonctions donnent les mêmes résultats. La variance expliquée par chaque axe est la même. 
```{r}
...
On va maintenant faire un graphique pour notre PCA avec les fonctions `ggord` du package **ggord**.
...
Le graphique créé par la fonction `ggord` est un objet ggplot. Cet objet (ici appelé pca.plot) peut être personnalisé, et peut ensuite servir de base pour continuer de créer le graphique en ajoutant (+) des couches avec ggplot.
```{r}
pca.plot <- ggord(pca.dune, xlims=c(-1,1), ylims=c(-1,1)) # xlims et ylims définissent les limites dans l'affichage des axe (les valeurs entre parenthèses correspondent à l'échelle numérique des axes).
pca.plot
```
...
On revient aux deux premiers axes. Faisons quelques petits changements.
```{r}
pca.plot <- ggord(pca.dune, xlims=c(-1.2,1.4), ylims=c(-0.75,0.9),
                  axes=c(1,2),
                  size=4, # taille des points (sites)
                  obslab=TRUE, # pour passer de points (par défaut) à texte
                  arrow=NULL,# enlever les flèches (par défaut) pour les espèces
                  txt=NULL, # enlever le texte (par défaut) pour les espèces
                  addpts=sp.scores.df, # Ajouter les points correspondant aux scores des espèces pour mieux choisir quelles espèces mettre en graphique
                  addsize=4, # taille des points (espèces)
                  ptslab=TRUE, # changer de points à texte
...
Il y a beaucoup d'espèces affichées au milieu et c'est illisible. En plus, les espèces au milieu ne contribuent pas à la distinction compositionnelle des sites. On va donc garder seulement n espèces dont les scores ont la valeur absolue la plus élevée, pour chacun des deux axes qu'on met en graphique (ici PC1 et PC2).

On va créer un nouveau dataframe contenant seulement ces espèces, en utilisant quelques fonctions de la librairie **dplyr**.
```{r}
...
pca.plot <- ggord(pca.dune, xlims=c(-1.2,1.4), ylims=c(-0.75,0.9),
                  ...
                  arrow=NULL,# enlever les flèches (par défaut) pour les espèces
                  txt=NULL, # enlever le texte (par défaut) pour les espèces
                  addpts=sp.scores.skim, # Ajouter les points des espèces aux plus hauts/bas scores
                  addsize=4, # taille des points (espèces)
                  ptslab=TRUE, # changer de points à texte
                  addcol="forestgreen") # couleur des espèces
                  #addpch=1,
...
vect$species<-rownames(vect)
```

```{r}
gg_envfit(ord = pca.dune, env = dune.env, perm = 999, pt.size = 2, alpha = 0.05)
```
...
pca.plot + 
  ...
  geom_text_repel(data = vect, aes(x = PC1, y = PC2, label = species, size = 3), show.legend = FALSE)
```



Maintenant, on va ajouter des ellipses autour des sites appartenant à des groupes similaires.
```{r}
pca.plot <- ggord(pca.dune, xlims=c(-1.2,1.4), ylims=c(-0.75,0.9),
                  axes=c(1,2),
                  grp_in=dune.env$Management, # ajout des ellipses regroupant les sites par type de Management
                  ...
                  alpha = 1)
pca.plot + scale_fill_brewer(palette="Accent") +
    coord_fixed() +
    geom_segment(data = vect,
               aes(x = 0, xend = PC1, y = 0, yend = PC2),
               arrow = arrow(length = unit(0.5, "cm")), colour = "darkgrey") +
...
Au lieu d'afficher le nom des sites, on pourrait vouloir afficher des points ou des symboles différents pour chaque type de groupement.
```{r}
pca.plot <- ggord(pca.dune, xlims=c(-1.2,1.4), ylims=c(-0.75,0.9),
                  axes=c(1,2),
                  grp_in=dune.env$Management, # ajout des ellipses regroupant les sites par type de Management
                  ...
                  addcol="black", # couleur des espèces
                  alpha = 1)
pca.plot + scale_shape_manual('Groups', values = c(15,16,17,18)) +# Assurer vous d'avoir le même nombre de valeur que de groupes  
  coord_fixed() +
  geom_segment(data = vect,
               aes(x = 0, xend = PC1, y = 0, yend = PC2),
               arrow = arrow(length = unit(0.5, "cm")), colour = "darkgrey") +
...
brewer.pal(n = 4, name = "Dark2") # Hexadecimal color specification 
```

Et on va incorporer ces codes de couleurs dans un argument + on déplace la légende vers le haut.
```{r}
pca.plot <- ggord(pca.dune, xlims=c(-1.2,1.4), ylims=c(-0.75,0.9),
                  axes=c(1,2),
                  grp_in=dune.env$Management, # ajout des ellipses regroupant les sites par type de Management
                  ...
                  addcol="black", # couleur des espèces
                  alpha = 1)
pca.plot <- pca.plot + scale_shape_manual('Groups', values = c(15,16,17,18)) +
    coord_fixed() + ## need aspect ratio of 1!
    geom_segment(data = vect,
               aes(x = 0, xend = PC1, y = 0, yend = PC2),
               arrow = arrow(length = unit(0.5, "cm")), colour = "darkgrey") +
...
```{r}
pca.plot <- pca.plot + annotate("text", x = 1.1, y = 0.7, label = "PERMANOVA\nR2=30%\n p=0.007") +
    super_theme
pca.plot
```


On sauvegarde l'image en png. Par défaut, gg 
```{r}
pca.plot
