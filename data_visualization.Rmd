...
À l'aide de la fonction `boxplot()` on peut visualiser la dispersion d'une variable selon différents groupes. Ce type de figure permet aussi d'identifier rapidement des valeurs aberrantes ou des anomalies. 

```{r}
boxplot(iris$Petal.Length~iris$Species)
```

... à modifier + continuer (Laurie)

Plusieurs analyses recquièrent que les résidus des données soint distribuées normalement.
Il est possible de vérifier visuellement la ditribution des résidus à l'aide de plusieurs
fonctions.
