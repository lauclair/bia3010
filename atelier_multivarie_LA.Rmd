...
=======
Rappel : les axes/vecteurs ne représente pas une variable biologique/écologique déterminée, mais peuvent être vu comme une "combinaison de variables". Contrairement à la PCA qui maximise la variance expliquée par le premier axe, **la CA maximise la correspondance entre les scores des sites et des espèces**. Le deuxième axe est perpendiculaire au 1er et calculé encore pour maximiser la correspondance entre les scores, et ainsi de suite... Les axes suivants sont orthogonaux entre eux (leur corrélation est nulle) et leur importance explicative de la variance va en diminuant.

On peut ensuite créer un graphique **biplot** avec la fonction `plot` pour visualiser les **sites** et les **espèces** le long des deux premiers axes. (étape 3)
>>>>>>> 6b501c51aea52eb51ae290e029bdd7a48e9646da
```{r}
...
La fonction plot effectue un scaling = 2 par défaut. Ce type de scaling est approprié si on cherche principalement à ordonner les espèces. Il conserve la distance chi2 entre les descripteurs (espèces) ainsi que la corrélation entre ceux-ci; l'angle entre les espèces représente leur corrélation (90° : aucune corrélation, 180° : forte corrélation négative). Une espèce proche d'un site est plus fréquent dans ce site qu'aux sites éloignées d'elle.

Sur un biplot avec un scaling = 1, la distance entre les objets (sites) reflète approximativement leur distance, mais l'angle entre les descripteurs (espèces) ne reflètent pas leur corrélation. Un site proche d'une espèce a une plus grande proportion de cette espèce que les sites éloignés d'elle.
...
" Ces scores permettent, par exemple, d'utiliser une composante principale comme une variable dans une autre analyse, ou de faire des graphiques supplémentaires. Par exemple, vous pouvez obtenir par PCA une variable unique issue du jeu de données “spe” puis l'utiliser pour la corréler par régression à une autre variable, ou déterminer un gradient spatial. Pour extraire les scores d'une PCA, utiliser la fonction scores (): "

```{r}
sp.scores<-scores(ca.varespec, display="species", choices=c(1,2)) # choices : les axes
site.scores<-scores(ca.varespec, display="sites", choices=c(1,2))
```

On va maintenant faire une régression (CA1 ~ Al)
```{r}
site.scores.df <- data.frame(site.scores)
varechem.df <- data.frame(varechem)
test <- lm(site.scores.df$CA1 ~ varechem.df$Al)
summary(test)
plot(test)
plot(site.scores.df$CA1 ~ varechem.df$Al)
abline(test$coefficient[1], test$coefficient[2])
```
...
--> La fonction **envfit** permet de calculer la régression (et donc la corrélation) de variables environnementales (indépendamment les unes des autres) aux axes d'une ordination. Cette fonction peut nous aider à interpréter écologiquement le positionnement des sites et des espèces dans l'ordination. 
Chaque variable environnementale est associée à une valeur p déterminée par permutation (donc change à chq fois qu'on la calcul) et à un R2.

```{r}
envfit.ca <- envfit(ca.varespec, varechem)
```

Les valeurs **CA1 et CA2** représentent la *direction des vecteurs* le long des deux axes de l'ordination, le **r2** donne la *force de la corrélation* entre les variables environnementales et les axes dans l'ordination et la **valeur P** [Pr(>r)] donne la *signification statistique*.
...
Attention! Les variables dont la corrélation est forte n'expliquent pas nécessairement la composition en espèces de nos sites, mais sont plutôt associées à la variation extraite par les axes de l'ordination. 
Ce test sert donc surtout à interpréter écologiquement les axes et les patrons d'ordination des espèces, d'émettre d'autres hypothèses, etc.
...
especes.wa <- wascores(varespec.pcoa$points[, 1:2], varespec) 
# Calculate weighted average projection of species
```
...
+ à utiliser si : On veut extraire le plus de variance possible avec nos axes; on veut donner moins de poids aux espèces rares dans la distinction de nos communautés; nos données contiennent plus de sites que d'espèces (vs PCoA).

## PCoA
+ données : quantitatives, qualitatives, mixtes
+ fonction : `cmdsacle` (librairie stats; base R)
...
+ bray-curtis : ignore les double zéros; donne plus de poids aux espèces communes vs chi2; affecté par la taille des échantillons (performe mieux avec faible diversité spécifique et petits échantillons).
...
Faire une ordination du même type pour chacune de vs matrices de communautés.

Le test Procruste superposera les deux ordinations.
