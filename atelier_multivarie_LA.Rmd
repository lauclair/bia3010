...
## Différents types d'analyses pour différentes questions

Tout comme pour les statistiques univariées, vous devrez choisir le bon test pour répondre à vos prédictions. 
...
Pour chacun de ces types de prédictions, nous verrons une méthode différente. Il existe différentes questions nécessitant différentes méthodes. 
 
Vous pourriez aussi vouloir tester si deux matrices d'abondances aux mêmes sites (ex. plantes et arthropodes) sont corrélées. Dans ce cas, vous pourriez faire des tests Procrustes. 
Nous ne verrons pas en détails ces tests ici, mais nous pourrons l'expliquer aux équipes intéressées.
...
Nos objets (sites) peuvent être ordonnés dans un espace multidimensionnel en fonction de leur ressemblance. Les sites situés plus proches l'un de l'autre dans cet espace sont plus similaires dans leur composition en espèces que les sites situés plus loin. Cet espace comporte autant de dimensions que de descripteurs (espèces). Il est donc très difficile de visualiser cet espace: comment représenter un espace qui a plus de 3 dimensions!
L'ordination permet de trouver les deux ou trois axes de notre espace qui explique le plus de variation entre nos sites. On peut donc faire une représentation 2D de cet espace.
...
Il existe différentes méthodes d'ordination: l'analyse en composantes principales (PCA), l'analyse de correspondance (CA), l'analyse en coordonnées principales (PCoA) et la mise à l'échelle multidimensionnelle (nMDS). Ici, nous verrons seulement l'analyse de correspondance puisqu'elle est bien adaptée aux données d'abondances.

# L'analyse de correspondance (CA)

Pour faire une ordination de type CA, il faut avoir une matrice d'abondance et toutes les données doivent être dans la même dimension physique (même unité) et ne pas contenir de données négatives, ni de NA.
Elle s'effectue en trois étapes dont voici le résumé (nous n'entrerons pas dans tous les détails ici):

1-Conversion de la matrice d'abondance en matrice de contributions de Pearson. 
(Cela permet notamment de diminuer le problème du double zéro: si vous avez deux sites qui ne possèdent pas une espèce de lichens A, cette absence d'espèce A ne contribuera pas autant à la ressemblance entre les 2 sites que la présence commune de l'espèce B aux 2 sites. La table de contribution de Pearson contient l'information pour ordonner les sites dans l'espace multidimensionnel.)
2-Décomposition de la table de contributions de Pearson en valeurs propres et en vecteurs propres. (Les vecteurs propres sont ce qui permet de réduire l'espace multidimensionnel en quelques axes qui expliquent le mieux la variation entre les sites.)
3-Visualisation en deux dimensions.

# Essayons

Avec le package vegan, on peut utiliser la fonction cca pour calculer une ordination CA.
...
Les eigenvalues (valeurs propres) représentent les axes de votre ordination. La première explique 25% de la variation dans vos données, la deuxième 17%, etc.(voir proportion explained). Ensuite, vous avez la position de vos objets et de vos descripteurs dans l'espace multidimensionnel à la manière d'une coordonnées (x,y,z,etc).
