...
Parfois, on aura besoin de remanipuler nos tableaux de données. Dans la librarie picante, la fonction **matrix2sample** converti une matrice de communautés en un tableau de données format "long" (chaque ligne est une observation). La fonction **sample2matrix** fait le contraire.
```{r}
library(picante)
...
À partir dune matrice de communauté, on peut aussi calculer des mesures de diversité alpha pour chaque site.
```{r}
...
Le premier axe sera toujours calculé pour représenter la plus grande variation dans l'espace multidimensionnel. Le deuxième axe est perpendiculaire au 1er et calculé encore pour maximiser la variance entre les points, et ainsi de suite... Les axes suivants sont orthogonaux entre eux (leur corrélation est nulle) et leur importance explicative de la variance va en diminuant.
...
Ici, le premier axe explique 25% de la variation dans vos données, le deuxième 17%, etc. Rappel : les axes/vecteurs ne représente pas une variable biologique/écologique déterminée, mais peuvent être vu comme une "combinaison de variables".
...
"**Biplot**" : En plus des *sites*, on peut aussi afficher les *espèces* sur notre ordination pour nous permettre de voir à quels sites ou groupe de sites elles sont associées.
```{r}
...
"En général, il est préférable d'avoir un plus grand nombre de sites que de variables dans le jeu de données (plus de lignes que de colonnes)." (réf : CSBQ atelier)
...
Allons inspecter en détails l'objet        qu'on vient de créer.
```{r}

```


L'inertie représente la variance totale de l'ordination qui est expliquée par les descripteurs (les espèces).
La proportion de la variance expliquée par un axe correspond à sa valeur propre divisé par l'inertie.
...
"La PCoA est aussi particulièrement adaptées pour les jeux de données présentant plus de colonnes que de lignes." (réf : CSBQ atelier)
...
Il peut être difficile de choisir quelle méthode d'ordination pour répondre à vos questions de recherche. De manière générale, pour le genre de données que vous avez récoltées (données d'abondances, peu de sites, beaucoup d'espèces, beaucoup de zéros), l'analyse de correspondance (CA) s'avère une méthode très puissante. Or, rien de mieux que de s'amuser à explorer et comparer les différentes méthodes.
