...
Les colonnes correspondent aux **descripteurs** (variables): il s'agit de vos espèces (ou de vos familles, morphotypes, etc.). Dans chaque cellule, on retrouve l'abondance (nombre d'individus, recouvrement, etc.) ou la présence-absence (0 ou 1) de chaque espèce à chaque site.
...
La CA s'emploi avec des **données d'abondances** brutes (sans pré-transformation) ou des **données binaires de présence-absence**.
...
La longueur d' vecteur renseigne sur la force de la corrélation (r2) entre la variable environnementale et les axes de l'ordination, tandis que la direction du vecteur fait pointer celui-ci vers les objets (sites) ayant des valeurs plus élevés pour cette variable.
