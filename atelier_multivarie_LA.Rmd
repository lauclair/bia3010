...
Lorsqu'on s'intéresse à des communautés à différents sites, on est souvent intéressé à visualiser quels sites sont plus semblables ou plus différents en termes de composition en espèces. Pour ce faire, on utilise souvent les ordinations. 
...
Les sites situés plus proches l'un de l'autre dans cet espace multidimensionel ellipsoïde sont plus similaires dans leur composition en espèces que les sites situés plus loin. 
...
--> L'ordination permet de trouver les deux ou trois axes de notre espace qui explique le plus de variation dans nos données. Ces axes représentent une "combinaison de variables" (et non plus des espèces particulières). L'ordination permet de visualiser l'espace multidimensionel en 2D (espace réduit) et simplifie l'interprétation des données multivariées.
...
Ici, on voit que les sites 3 et 4 ont une composition en espèces relativement similaire où on retrouve plus fréquemment l'espèce Flaniva, et dont le sol contient plus d'aluminium et de fer que les sites dans le quadrant en haut à droite.
...
# PERMANOVA : L'analyse permutationnelle de variance multivariées 
