...
Comme la CA, les données qu'on fournit pour une PCA sont sous forme de *matrice de communautés*. Mais, contrairement à la CA, la PCA peut être utilisée avec des **données d'abondance**, mais n'est pas la méthode la plus appropriée pour travailler avec des données de présence-absence.
...
Il peut être difficile de choisir quelle méthode d'ordination pour répondre à vos questions de recherche. De manière générale, pour le genre de données que vous avez récoltées, la CA et la PCA s'avèrent des méthodes très adaptées. Or, rien de mieux que de s'amuser à explorer et comparer les différentes méthodes!
...
+ à utiliser si : On veut extraire le plus de variance possible avec nos axes; on veut donner moins de poids aux espèces rares dans la distinction de nos communautés; nos données contiennent plus de sites que d'espèces
...
+ à utiliser si : Nos données contiennent plus d'espèces que de sites; on veut utiliser une des nombreuses autres distances que euclidienne et chi-carré en fonction de nos données; on a des données qualitatives.
