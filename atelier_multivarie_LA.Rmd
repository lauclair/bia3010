...
Chaque axe de l'ordination est un eigenvector (vecteur propre), qui est associé à une eigenvalue (valeur propre). Dans la CA, les valeurs propres correspondent à "l'inertie extraite" (or equivalently, correlation coefficients).
...
"Proportion explained" : Cette valeur représente la proportion de la variation totale dans la composition des communautés qui est expliqué par un axe donné. Ici, le premier axe explique 25% de la variation dans vos données, le deuxième 17%, etc. Rappel : les axes ne représente pas une variable biologique/écologique déterminée, mais peuvent être vu comme une "combinaison de variables".

"Cumulative proportion" : Proportion cumulative d'une axe donné et des précédents.

Ensuite, vous avez les coordonnées (la position) de vos objets ("**site scores**") et de vos descripteurs ("**species scores**") dans l'espace multidimensionnel.
...
"**Biplot**"On peut aussi afficher les espèces sur notre ordination pour nous permettre de voir à quels sites ou groupe de sites elles sont associées.
```{r}
